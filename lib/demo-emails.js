...
async function sendDemoEmails(toEmail) {
  ...
  const from = 'Antortiq <onboarding@resend.dev>';
  const templates = [emailShipped(), emailOfd(), emailReturnVendor()];
  for (const t of templates) {
    console.log(`[demo-email] sending: ${t.subject}`);
    const { data, error } = await resend.emails.send({ from, to: toEmail, subject: t.subject, html: t.html });
    if (error) throw new Error(error.message);
    console.log(`[demo-email] sent: ${data.id}`);
